...
export function useOnboardingForm() {
  const validateProfileForm = useCallback(
    (data: ProfileFormData): ProfileFormErrors => {
      ...
      if (
        ...
        data.expected_return_pct < -100 ||
        data.expected_return_pct > 1000
      ) {
        errors.expected_return_pct =
          "Oczekiwany zwrot musi być w zakresie -100 do 1000";
      }
      ...
      if (data.birth_date) {
        ...
        if (date >= today) {
          ...
          errors.birth_date =
            "Data urodzenia nie może być starsza niż 120 lat";
        }
      }
  ...
  const validateInvestmentForm = useCallback(
    (data: InvestmentFormData): InvestmentFormErrors => {
      const errors: InvestmentFormErrors = {};

      // Validate type
      if (
        !data.type ||
        !["etf", "bond", "stock", "cash"].includes(data.type)
      ) {
      ...
      if (
        ...
        errors.amount =
          "Kwota musi być większa od 0 i mniejsza niż 999999999999.99";
      }
